...
import Header from "../components/Header"; // ✅ Agregar Header
import Footer from "../components/Footer"; // ✅ Agregar Footer

const FavouritePage = () => {
  const [favourites, setFavourites] = useState([]);
  const [loading, setLoading] = useState(true);
  const userId = 11; // ✅ ID del usuario (puedes cambiarlo dinámicamente)
  ...
  const fetchFavorites = async () => {
    try {
      ...
      console.log("Datos recibidos del backend:", response.data); // 🔍 Ver en consola
  ...
  const removeFavourite = async (productId) => {
    try {
      ...
      setFavourites(favourites.filter((product) => product.product_id !== productId));
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-amber-100 text-amber-900 overflow-x-hidden">
      ...
      <main className="flex-1 flex flex-col items-center p-6 text-center">
        <h1 className="text-4xl font-extrabold mb-4">Tus Productos Favoritos</h1>
        ...
        {loading ? (
          ...
          <p className="text-lg text-gray-500">No tienes productos favoritos.</p>
        ) : (
          <div className="w-full max-w-4xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {favourites.map((product) => (
              <div key={product.product_id} className="relative bg-white p-6 rounded-lg shadow-md text-center">
                <button
                  onClick={() => removeFavourite(product.product_id)}
                ...
                <img src={product.product.image} alt={product.product.name} className="w-full h-auto rounded-lg mb-4" />
                <h2 className="text-2xl font-bold mb-2">{product.product.name}</h2>
                <p className="text-lg text-amber-800 mb-2">Precio: {product.product.price}€</p>
